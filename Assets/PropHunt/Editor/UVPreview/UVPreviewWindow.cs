...
//	private int gridStep = 16;
	...
	void OnGUI () {
        ...
        //if(selected != null)
          //  Debug.Log("selected "+selected.name + " " + selected.GetType());
	    if (selected != null && selected is Mesh)
        {
            m = selected as Mesh;
            selectedObject = null;
            ...
            if(selectedObject != null) 
            {
                if(selectedObject.GetComponent<MeshFilter>() != null || selectedObject.GetComponent<SkinnedMeshRenderer>() != null)
                {
                    ...
                    _lightmapRenderer = selectedObject.GetComponent<MeshRenderer>();
                } 
                else{
                    _lightmapRenderer = null;
		...
		if(m == null){
            ...
            else EditorGUILayout.HelpBox("Selected mesh: " + m.name, MessageType.None);
			...
			if(m.uv2.Length > 0){
                if(_lightmapRenderer != null && !Application.isPlaying)
                    selectedUVStrings = new string[3];
                    else selectedUVStrings = new string[2];
                selectedUVStrings[0] = "UV";
                selectedUVStrings[1] = "UV 2";
                    if(_lightmapRenderer != null && !Application.isPlaying)
                    selectedUVStrings[2] = "Lightmap "+_lightmapRenderer.lightmapIndex;
                ...
                else 
                {
                    ...
                    {
                        var uv2 = m.uv2;
                        var lightmapTiling = new Vector2(_lightmapRenderer.lightmapScaleOffset.x,_lightmapRenderer.lightmapScaleOffset.y);
                        var lightmapOffset = new Vector2(_lightmapRenderer.lightmapScaleOffset.z,_lightmapRenderer.lightmapScaleOffset.w);
                        uvs = new Vector2[uv2.Length];

                        for ( int i =0 ; i < uvs.Length ; i++)
                        {
                            uvs[i] = Vector2.Scale( uv2[i],lightmapTiling)+lightmapOffset;
                        }
                    }
                }
			}
		...
		/*	for(int i = -2048; i < 2048; i+= (int)(gridStep)){
            ...
            for( int i = -100; i <= 100; i ++)
            {
                ...
                float step = (float)i/10.0f;
                int vgridX = (int)(step*scale*windowDefaultSize+sideSpace+xPanShift);
                int vgridy1 = ySpace+sideSpace;
                int vgridy2 = (int)(uvPreviewRect.height+ySpace+sideSpace);

                if(vgridX > (int)(uvPreviewRect.x-1) &&
                    vgridX <(int)(uvPreviewRect.width+sideSpace))
                    DrawLine(vgridX, vgridy1,vgridX,vgridy2,color);
            }
            ...
            for( int i= -100; i <= 100; i ++)
            {
                ...
                int hgridY = (int)(-(step*scale*windowDefaultSize)+ySpace+sideSpace+yPanShift)+windowDefaultSize;
                int hgridX1 = (int)(uvPreviewRect.x-1);
                int hgridX2 = (int)(uvPreviewRect.width+sideSpace);
                if(hgridY > ySpace+sideSpace &&
                   hgridY <  (int)(uvPreviewRect.height+ySpace+sideSpace))
                    DrawLine(hgridX1, hgridY,hgridX2,hgridY,color);
            }
			//UV
			...
			if(GUILayout.Button("Save To PNG")){
				...
				uvSaveWindow.titleContent = new GUIContent("Save to PNG");
